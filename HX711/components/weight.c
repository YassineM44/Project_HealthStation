...
static esp_err_t hx711_wait_ready(const weight_sensor_t *sensor, uint32_t timeout_us)
{
                                       ...
                                       int32_t *average)
{
    ...
    if ((sample_count == 0U) || (average == NULL) || (sample_count > HX711_MAX_SAMPLE_BUFFER)) {
 ...
 */
static int32_t weight_half_window_avg(const weight_sensor_t *sensor, bool recent)
{
    int64_t sum = 0;
    ...
    for (uint8_t i = 0; i < HALF_WINDOW; i++) {
        ...
        uint8_t idx = (sensor->history_index + WEIGHT_HISTORY_SIZE
                       - 1 - start_offset - i) % WEIGHT_HISTORY_SIZE;
...
esp_err_t weight_init(weight_sensor_t *sensor, const weight_config_t *config)
{
    ...
    if (!GPIO_IS_VALID_GPIO(config->dout_gpio) || !GPIO_IS_VALID_GPIO(config->sck_gpio)) return ESP_ERR_INVALID_ARG;
    if ((config->samples_per_update == 0U) || (config->settle_reads_required == 0U)) return ESP_ERR_INVALID_ARG;
    if ((config->present_sign < -1) || (config->present_sign > 1)) return ESP_ERR_INVALID_ARG;
    ...
    ESP_LOGI(TAG, "HX711 init DOUT=%d SCK=%d thresh=%" PRIu32,
             config->dout_gpio, config->sck_gpio, config->present_threshold_raw);
    return ESP_OK;
}
...
{
    int32_t baseline = 0;
    if ((sensor == NULL) || !sensor->initialized || (sample_count == 0U)) return ESP_ERR_INVALID_ARG;

    esp_err_t err = hx711_average_samples(sensor, sample_count, &baseline);
    if (err != ESP_OK) return err;

    /* Fill entire history with the tare value */
    for (uint8_t i = 0; i < WEIGHT_HISTORY_SIZE; i++) {
        sensor->history[i] = baseline;
    }
    sensor->history_index = 0;
    sensor->history_count = 0;
    sensor->pill_present = false;
    sensor->streak = 0;
    sensor->cooldown = WEIGHT_HISTORY_SIZE; /* let thermal settling pass */
    sensor->tared = true;

    ESP_LOGI(TAG, "Tare complete: baseline=%" PRId32, baseline);
    return ESP_OK;
}
...
esp_err_t weight_update(weight_sensor_t *sensor, weight_status_t *status)
{
    ...
    if ((sensor == NULL) || (status == NULL)) return ESP_ERR_INVALID_ARG;
    if (!sensor->initialized || !sensor->tared) return ESP_ERR_INVALID_STATE;

    esp_err_t err = hx711_average_samples(sensor, sensor->config.samples_per_update, &raw_average);
    if (err != ESP_OK) return err;
    ...
    int32_t recent_avg = weight_half_window_avg(sensor, true);
    int32_t older_avg  = weight_half_window_avg(sensor, false);
    int32_t window_delta = recent_avg - older_avg;

    int32_t active_delta = window_delta;
    if (sensor->config.present_sign == 1) {
        active_delta = window_delta;
    } else if (sensor->config.present_sign == -1) {
        active_delta = -window_delta;
    } else {
        active_delta = (window_delta > 0) ? window_delta : -window_delta;
    }

    uint32_t thresh = sensor->config.present_threshold_raw;

    /* Cooldown: after a state change, wait for the full buffer to cycle
     * with the new level before checking again */
    if (sensor->cooldown > 0) {
        sensor->cooldown--;
    } else if (active_delta >= (int32_t)thresh) {
        sensor->streak++;
        if (sensor->streak >= sensor->config.settle_reads_required) {
            bool new_state = !sensor->pill_present;
            sensor->pill_present = new_state;
            sensor->streak = 0;
            sensor->cooldown = WEIGHT_HISTORY_SIZE; /* wait for full buffer cycle */
            ESP_LOGI(TAG, "State changed: pill=%s (delta=%" PRId32 ", th=%" PRIu32 ")",
                     new_state ? "true" : "false", active_delta, thresh);
        }
    } else {
        sensor->streak = 0;
    }
