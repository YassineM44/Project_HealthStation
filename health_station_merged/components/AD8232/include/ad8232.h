 ...
 *
 * Provides hardware-calibrated ADC acquisition at 500 Hz with an 8-stage
 * IIR digital signal processing pipeline:
 *   1-2. 4th-order Butterworth high-pass @ 0.67 Hz (baseline wander)
 *   3.   50 Hz notch Q=30 — power-line interference (EU)
 *   4.   60 Hz notch Q=30 — power-line interference (US)
 *   5-6. 4th-order Butterworth low-pass @ 18 Hz (EMG / HF noise)
 *   7.   3-point median filter (impulse spike removal)
 *
...
typedef struct {
    ...
    /* DSP filter bank — order: HP×2 → notch50 → notch60 → LP×2 → median */
    ad8232_biquad_t hp1;            /**< 4th-order HP section 1  */
    ad8232_biquad_t hp2;            /**< 4th-order HP section 2  */
    ad8232_biquad_t notch50;        /**< 50 Hz notch             */
    ad8232_biquad_t notch60;        /**< 60 Hz notch             */
    ad8232_biquad_t lp1;            /**< 4th-order LP section 1  */
    ad8232_biquad_t lp2;            /**< 4th-order LP section 2  */

    /* 3-point median filter for impulse noise */
    float med_buf[AD8232_MEDIAN_LEN]; /**< Circular buffer       */
    uint8_t med_idx;                  /**< Next write position    */
    bool    med_primed;               /**< Buffer fully filled    */
} ad8232_handle_t;
